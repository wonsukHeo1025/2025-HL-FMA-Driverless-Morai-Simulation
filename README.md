...
이 저장소는 MORAI 시뮬레이터 기반 자율주행 워크스페이스입니다.
...
Localization, Dead Reckoning, Low-level Control은 별도 담당 영역이었기 때문에 여기서는 설명하지 않습니다.
...
- 차선 수에 맞는 개수의 전역 후보 경로 발행
- LiDAR로 장애물을 인지하고 더 안전한 후보 경로 선택
- 신호등 상태를 인지하고 필요 시 정지선애서 정차
- 최종 선택된 경로와 목표 속도 프로파일을 Low 제어 스택에 전달
...
이 패키지는 Raw LiDAR 데이터를 실제 주행 판단에 사용할 수 있는 장애물 정보로 변환합니다.
...
이 패키지는 판단의 중심 역할을 합니다. 다중 GPS 경로를 관리하고, 경로 점유도를 평가하며, 최종 주행 경로를 선정합니다.
